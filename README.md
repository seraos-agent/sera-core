...
[![Tests](https://img.shields.io/badge/Tests-148%20passed%20(100%25)-10b981.svg?style=flat-square)](https://github.com/seraos-agent/sera-core)
...
- **19 Built-In Tools**: Full suite of agent actions covering chat, memory management, vault transfers, Hyperliquid spot trading, in-conversation proposal approvals, Google Drive rich XLSX creation, document append, file deletion, Threads publishing, and temporal scheduling.
...
- **Global Multi-Currency XLSX Engine**: Generates executive-ready `.xlsx` workbooks with frozen top pane, status badge pills, dark navy headers, smart multi-currency detection (`₹` INR, `$` USD, `€` EUR, `£` GBP, `¥` JPY, `S$` SGD, `RM` MYR, `Rp` IDR), and live SUM formulas.
- **Document Append & Vault Hygiene**: Incremental notes/log appending and safe obsolete file deletion (`sera_gdrive_delete`).
...
├── tests/                     # 148 unit, integration, and simulation tests (100% pass)
