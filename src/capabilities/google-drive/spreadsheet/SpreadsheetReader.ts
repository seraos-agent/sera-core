...
export class SpreadsheetReader {
  ...
  public static async readWorkbookAsMarkdown(buffer: Buffer): Promise<string> {
    ...
    if (!workbook.worksheets || workbook.worksheets.length === 0) return '(Empty Spreadsheet)';
    ...
    for (const worksheet of workbook.worksheets) {
      ...
      const getCellNumeric = (colLetter: string, rowNum: number): number => {
        ...
        if (typeof val === 'object') {
          ...
          if ('formula' in val) {
            const evalRes = SpreadsheetFormulaEngine.evaluateRowFormula(String(val.formula), matrix[rIdx] || [], rowNum);
            if (evalRes !== undefined) {
              matrix[rIdx][cIdx] = evalRes;
              return evalRes;
            }
          }
        }
        if (typeof val === 'string' && val.trim().startsWith('=')) {
          const evalRes = SpreadsheetFormulaEngine.evaluateRowFormula(val.trim(), matrix[rIdx] || [], rowNum);
          if (evalRes !== undefined) {
            matrix[rIdx][cIdx] = evalRes;
            return evalRes;
          }
          ...
          if (val !== null && typeof val === 'object' && 'formula' in val) {
            ...
            if (sumMatch) {
              ...
              if (avgMatch) {
                ...
                if (typeof val === 'number') matrix[rIdx][cIdx] = val;
              } else {
      ...
      const sheetTitle = worksheet.name || 'Sheet1';
      renderedSheets.push([
        `### Sheet: ${sheetTitle}`,
        headerLine,
        separatorLine,
    ...
    if (renderedSheets.length === 0) return '(Empty Spreadsheet)';
    return renderedSheets.join('\n\n');
  }
}
