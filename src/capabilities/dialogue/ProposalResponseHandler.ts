...
export class ProposalResponseHandler {
  ...
  public isApproval(message: string): boolean {
    return /^(?:yes|y|iya|ya|approve|approved|proceed|confirm|ok|okay|oke|kerjakan|jalankan|mulai|gas|lanjut|lakukan|setuju|acc|proses|gaskan|eksekusi|hapus|delete)(?:\s+(?:kerjakan|jalankan|mulai|lakukan|proses|gaskan|sekali|dong|ya|aja|saja|post|postingan|sekarang|ini))?[.!\s]*$/i.test(message.trim());
  }

  public isRejection(message: string): boolean {
    return /^(?:no|n|tidak|batal|cancel|reject|deny|stop|jangan|gak|ga|tiada|nggak)(?:\s+(?:jadi|usah|perlu|deh|dulu|batal))?[.!\s]*$/i.test(message.trim());
  }
