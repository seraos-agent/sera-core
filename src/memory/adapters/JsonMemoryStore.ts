...
export class JsonMemoryStore implements IMemoryStore {
  ...
  constructor(private eventBus?: EventEmitter) {
    ...
    this.policyEngine = new MemoryPolicyEngine(this);
  ...
  private load() {
    if (fs.existsSync(this.filePath)) {
      try {
        ...
        console.error('[JsonMemoryStore] Failed to load memory', e);
      }
    }
  }
  ...
  storeBelief(belief: Belief): void {
    ...
    if (belief.key && belief.key.startsWith('wallet.')) {
      console.log(`[JsonMemoryStore] Intercepting ${belief.key} for Policy Evaluation...`);
  ...
  public __mutate_protected(proposal: MemoryProposal, newStatus: MemoryStatus, verification: VerificationLevel): any {
    ...
    console.log(`[JsonMemoryStore] Mutated protected belief: ${proposal.key} -> [${newItem.status}]`);
    return newItem;
  }
}
