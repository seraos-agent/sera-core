...
export function ConnectionsPage({ theme, walletState: _walletState, onBack, isMobileView, socket, threads, onConnectThreads, onDisconnectThreads, telegram, telegramLinkCode, onGenerateTelegramLink, googleDrive, onConnectGoogleDrive, onDisconnectGoogleDrive }: WorkspacePageProps) {
  ...
  useEffect(() => {
    // Pre-generate Telegram link so the button is instantly ready to redirect
    if (telegram?.status !== 'CONNECTED' && !telegramLinkCode && onGenerateTelegramLink) {
      onGenerateTelegramLink();
    }
  }, [telegram?.status, telegramLinkCode, onGenerateTelegramLink]);

  const handleActivate = (connectorId: string) => {
    if (connectorId === 'threads') {
      if (onConnectThreads) onConnectThreads();
      else if (socket) socket.emit('threads:connect');
      return;
    }
    if (connectorId === 'telegram') {
      if (onGenerateTelegramLink) onGenerateTelegramLink();
  ...
  const handleDeactivate = (connectorId: string) => {
    if (connectorId === 'threads') {
      if (onDisconnectThreads) onDisconnectThreads();
      else if (socket) socket.emit('threads:disconnect');
      return;
    }
    if (connectorId === 'telegram') {
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ width: 42, height: 42, borderRadius: 14, background: theme.surface, border: `1px solid ${theme.border}`, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
            {connector.id === 'wallet' ? (
              ...
              <img src="/hyperliquid.png" style={{ width: "100%", height: "100%", objectFit: "contain", padding: 4 }} />
        ...
        {!connector.alwaysActive && (
          <div style={{ marginTop: 4 }}>
            {isConnectorActive ? (
              ...
              <a href={telegramLinkCode ? `https://t.me/sera_osbot?start=${telegramLinkCode}` : "#"} target={telegramLinkCode ? "_blank" : undefined} rel="noopener noreferrer"
                onClick={(e) => {
                  if (!telegramLinkCode) {
                    e.preventDefault();
                ...
                style={{
                  width: "100%", padding: "8px 0", borderRadius: 10,
                  border: "none", background: theme.ink,
                  color: theme.bg, fontSize: 13, fontWeight: 600, cursor: "pointer",
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
              ...
              <button
                onClick={() => handleActivate(connector.id)}
                style={{
                  width: "100%", padding: "8px 0", borderRadius: 10,
                  border: "none", background: theme.ink,
                  color: theme.bg, fontSize: 13, fontWeight: 600, cursor: "pointer",
