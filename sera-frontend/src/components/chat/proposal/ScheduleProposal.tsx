    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, marginTop: 8 }}>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, zIndex: 1 }}>
          ...
          <span style={{ fontSize: 12, color: theme.inkSoft, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase' }}>Scheduled Event</span>
        </div>
        
        <span style={{ fontSize: 24, color: theme.ink, fontWeight: 800, letterSpacing: -0.5, zIndex: 1, textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16, borderRadius: 12, background: theme.isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)', border: `1px solid ${theme.border}` }}>
        ...
        {isTransfer && p.actionParameters?.amount && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
            ...
            <span style={{ fontSize: 14, color: theme.ink, fontWeight: 700 }}>
              {p.actionParameters.amount === 'all' ? 'Entire Balance' : p.actionParameters.amount} {p.actionParameters.asset?.toUpperCase()}
            </span>
          </div>
        )}
      </div>
